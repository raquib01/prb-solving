...
void arrRev(char arr[],int arrLen){
    int temp;
...
int main(){
	...
	char input_str[MAX] = "a+b*C+d";
	int arrLen = 7;
	arrRev(input_str,arrLen); //reversing input string
	char output_str[MAX];
	...
	arrRev(output_str,arrLen);//reversing output string
	printf("Prefix: %s\n",output_str);
	return  0;
}